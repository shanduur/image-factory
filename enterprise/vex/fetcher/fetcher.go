...
import (
	...
	"github.com/siderolabs/go-vex/pkg/types"
...
func NewDataFetcher(registry string, refreshInterval time.Duration, remoteOpts ...remote.Option) (*DataFetcher, error) {
...
func (f *DataFetcher) Fetch(ctx context.Context, version string) (*types.ExploitabilityData, error) {
	version = remoteTag(version)

	ref, err := name.NewTag(fmt.Sprintf("%s:%s", f.registry, version))
	if err != nil {
	...
	if err != nil {
		return nil, fmt.Errorf("error reading VEX data: %w", err)
	}

	return types.LoadExploitabilityData(bytes.NewReader(data))
}
